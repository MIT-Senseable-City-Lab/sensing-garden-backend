...
def test_cli_clips_dot_frame_bboxes_to_fake_canvas(tmp_path: Path) -> None:
    results_key = "v1/FLIK2-dot01/20260413/results.json"
    _write_json(
        tmp_path,
        results_key,
...
def test_cli_skips_dot_frame_bboxes_outside_fake_canvas(tmp_path: Path, capsys: CaptureFixture[str]) -> None:
    ...
    composite_path = tmp_path / "v1/FLIK2-dot01/20260413/composites/157868_184852.jpg"
    assert exit_code == 0
    assert not composite_path.exists()
    assert payload[0]["status"] == "skipped"
    assert payload[0]["reason"] == "no_boxes"
...
def test_cli_overwrites_existing_composite_when_flag_set(tmp_path: Path, capsys: CaptureFixture[str]) -> None:
    results_key = "v1/FLIK2-dot01/20260412/results.json"
    composite_key = "v1/FLIK2-dot01/20260412/composites/12224_163315.jpg"
    _write_json(
        tmp_path,
    ...
    _write_jpeg(tmp_path, "v1/FLIK2-dot01/20260412/crops/12224_163315/frame_000000.jpg", "blue")
    _write_jpeg(tmp_path, composite_key, "white")
    ...
    payload = json.loads(capsys.readouterr().out)
    composite = Image.open(tmp_path / composite_key)
    assert exit_code == 0
